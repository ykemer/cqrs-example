...
describe('PATCH /api/v1/courses/:id', () => {
  describe('Error cases', () => {
    it('should return 400 when id is not a valid UUID', async () => {
      ...
      const res = await request(app).patch('/api/v1/courses/not-a-uuid').send({name: 'N', description: 'D'});
    ...
    it('should return 400 when name is missing', async () => {
      ...
      const app = createTestApp({currentUser: {id: 'a', role: UserRole.admin}});
      const res = await request(app).patch(`/api/v1/courses/${course.id}`).send({description: 'D'});
      expect(res.status).toBe(400);
    });
    ...
    it('should return 401 when the user is unauthenticated', async () => {
      ...
      const res = await request(app).patch(`/api/v1/courses/${course.id}`).send({name: 'N', description: 'D'});
    ...
    it('should return 404 when the course does not exist', async () => {
      const app = createTestApp({currentUser: {id: 'a', role: UserRole.admin}});
      const res = await request(app).patch(`/api/v1/courses/${GUID1}`).send({name: 'N', description: 'D'});
